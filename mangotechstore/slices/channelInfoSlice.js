...
const initialState = {
  ...
  export const fetchChannelInfo = createAsyncThunk(
    ...
    async () => {
      const response = await axios.get(`https://api.twitch.tv/helix/channels?broadcaster_id=${process.env.NEXT_PUBLIC_TWITCH_CHANNEL_ID}`,
      {
        headers: {
...
export const { pending: fetchChannelInfoPending, fulfilled: fetchChannelInfoFulfilled, rejected: fetchChannelInfoRejected } = fetchChannelInfo;

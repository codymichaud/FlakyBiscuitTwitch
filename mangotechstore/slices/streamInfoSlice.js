...
export const fetchStreamInfo = createAsyncThunk(
  ...
  async () => {
    const getStreams = axios.get(
      `https://api.twitch.tv/helix/videos?user_id=${process.env.NEXT_PUBLIC_TWITCH_CHANNEL_ID}`,
      {
        headers: {
...
export const {
  pending: fetchStreamInfoPending,
  fulfilled: fetchStreamInfoFulfilled,
  rejected: fetchStreamInfoRejected,
} = fetchStreamInfo;
